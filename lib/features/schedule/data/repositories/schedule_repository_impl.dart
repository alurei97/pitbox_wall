...
class ScheduleRepositoryImpl implements ScheduleRepository {
  ...
  @override
  Future<Either<Failure, List<Race>>> getSeasonSchedule({bool forceRefresh = false}) async {
    if (!forceRefresh) {
      final fresh = await _db.getFresh(_cacheKey);
      if (fresh != null) {
        try {
          return Right(_decodeRaces(fresh.jsonData));
        } catch (_) {
  ...
  Future<Either<Failure, List<Race>>> _staleOrFailure(Failure failure) async {
    ...
    if (stale != null) {
      try {
        return Right(_decodeRaces(stale.jsonData));
      } catch (_) {
